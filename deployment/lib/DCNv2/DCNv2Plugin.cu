...
#define KERNEL_POSITION \
	...
	int position = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;\
...
void enqueue_call(
	...
	if (mType == nvinfer1::DataType::kFLOAT){
		enqueue_native<float>(mCublasHandle, batchSize, static_cast<const float*>(inputs[0]),
		...
		enqueue_native<half>(mCublasHandle, batchSize, static_cast<const half*>(inputs[0]),
